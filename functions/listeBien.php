...
function updateAnnonce(string $titre,string $type,string $adresse,string $description,int $nb_voyageurs,int $nb_chambre,int $prix,string $photobdd,int $id_annonce) {
  $pdo=getPdo();
  $query= "UPDATE annonces SET titre=:titre, type=:type, adresse=:adresse, description=:description, nb_voyageurs=:nb_voyageurs, nb_chambre=:nb_chambre, prix=:prix, photo=:photo WHERE id=:id;";
  $stmt = $pdo->prepare($query);
